...
public interface Configuration331Sol002Api {
	...
	@Operation(summary = "Read VNF/VNFC configuration from VNF", description = "The client can use this method to read configuration information about a VNF instance and/or its VNFC instances. ", tags = {})
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "200 OK Shall be returned when configuration information about a VNF instance  has been read successfully. The response body shall contain a representation of the configuration resource. ", content = @Content(schema = @Schema(implementation = VnfConfiguration.class))),
	...
	@Operation(summary = "Modify VNF/VNFC configuration.", description = "This method sets or modifies a configuration resource.", tags = {})
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "200 OK Shall be returned when the request has been accepted and completed.  The response body shall contain the parameters of the configuration  modification that was applied to the configuration resource. ", content = @Content(schema = @Schema(implementation = VnfConfigModifications.class))),
	...
	default ResponseEntity<VnfConfigModifications> configurationPatch(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.DEFAULT, description = "The parameter for the configuration modification, as defined in clause 9.5.2.2.", required = true, schema = @Schema()) @Valid @RequestBody final VnfConfigModifications body, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
			if (getAcceptHeader().get().contains("application/json")) {
				try {
					return new ResponseEntity<>(getObjectMapper().get().readValue(
