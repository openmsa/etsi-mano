 ...
 * defined in table 5.5.2.2-1. NOTE: Clause B.3.2 provides examples illustrating
 * the relationship among the different run-time information elements (CP, VL
 * and link ports) used to represent the connectivity of a VNF. NOTE 1:
 ...
 * NOTE 2: ETSI GS NFV-SOL 001 specifies the structure and format of the VNFD
 * based on TOSCA specifications. NOTE 3: VNF configurable properties are
 * sometimes also referred to as configuration parameters applicable to a VNF.
 * Some of these are set prior to instantiation and cannot be modified if the
 * VNF is instantiated, some are set prior to instantiation (are part of initial
 * configuration) and can be modified later, and others can be set only after
 * instantiation. The applicability of certain configuration may depend on the
 * VNF and the required operation of the VNF at a certain point in time. NOTE 4:
 * Upon creation of the VnfInstance structure, the VNFM shall create and
