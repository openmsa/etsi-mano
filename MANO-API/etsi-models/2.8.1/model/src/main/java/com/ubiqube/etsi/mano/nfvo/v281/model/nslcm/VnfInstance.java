...
public class VnfInstance {
	...
	public VnfInstance vnfdId(final UUID vnfdId) {
		this.vnfdId = vnfdId;
		return this;
	}

	/**
	 * Identifier of the VNFD on which the VNF instance is based. Modifications to
	 * this attribute can be requested using the \"ModifyVnfInfoData\" structure.
	 * Modifying the value of this attribute shall not be performed when conflicts
	 * exist between the previous and the newly referred VNF package, i.e. when the
	 * new VNFD is changed with respect to the previous VNFD in other aspects than
	 * merely referencing to other VNF software images. In order to avoid
	 * misalignment of the VnfInstance with the current VNF's on-boarded VNF
	 * Package, the values of attributes in the VnfInstance that have corresponding
	 * attributes in the VNFD shall be kept in sync with the values in the VNFD.
	 *
	 * @return vnfdId
	 **/
	...
	public void setVnfdId(final UUID vnfdId) {
		this.vnfdId = vnfdId;
	}
	 ...
	 * which the VNF is based. This identifier has been allocated by the NFVO.
	 *
	 * @return vnfPkgId
	 **/
	 ...
	 * false, auto-healing is currently disabled. Modifications to these attributes
	 * can be requested using the \"ModifyVnfInfoData\" structure.
	 *
	 ...
	 * represented in the VnfInstance structure. Metadata that the VNF provider
	 * foresees are expected to be declared in the VNFD (see note 2). Modifications
	 * to these attributes can be requested using the \"ModifyVnfInfoData\"
	 * structure. NOTE 2: ETSI GS NFV-SOL 001 specifies the structure and format of
	 ...
	 * instance. All extensions that are allowed for the VNF are declared in the
	 * VNFD. Modifications to these attributes can be requested using the
	 * \"ModifyVnfInfoData\" structure.
	 *
	 * @return extensions
	 **/
