...
public class Indexer extends SpelNodeImpl {
	...
	@Override
	public void setValue(final ExpressionState state, @Nullable final Object newValue) throws EvaluationException {
	...
	protected ValueRef getValueRef(final ExpressionState state) throws EvaluationException {
		...
		final Object target = context.getValue();
		final TypeDescriptor targetDescriptor = context.getTypeDescriptor();
		...
		if (target.getClass().isArray() || (target instanceof Collection) || (target instanceof String)) {
			final int idx = (Integer) state.convertValue(index, TypeDescriptor.valueOf(Integer.class));
		...
		final TypeDescriptor valueType = indexValue.getTypeDescriptor();
	...
	@Override
	public void generateCode(final MethodVisitor mv, final CodeFlow cf) {
		...
		if (this.indexedType == IndexedType.ARRAY) {
			...
			final SpelNodeImpl index = this.children[0];
			...
			if ((this.children[0] instanceof PropertyOrFieldReference)) {
				final PropertyOrFieldReference reference = (PropertyOrFieldReference) this.children[0];
				final String mapKeyName = reference.getName();
			...
			final ReflectivePropertyAccessor.OptimalPropertyAccessor accessor = (ReflectivePropertyAccessor.OptimalPropertyAccessor) this.cachedReadAccessor;
			Assert.state(accessor != null, "No cached read accessor");
			final Member member = accessor.member;
			final boolean isStatic = Modifier.isStatic(member.getModifiers());
			final String classDesc = member.getDeclaringClass().getName().replace('.', '/');
	...
	private Object accessArrayElement(final Object ctx, final int idx) throws SpelEvaluationException {
		...
		if (arrayComponentType == Boolean.TYPE) {
			...
			final Object[] array = (Object[]) ctx;
			checkAccess(array.length, idx);
			final Object retValue = array[idx];
	...
	private <T> T convertValue(final TypeConverter converter, @Nullable final Object value, final Class<T> targetType) {
		final T result = (T) converter.convertValue(
	...
	private class ArrayIndexingValueRef implements ValueRef {
		...
		public TypedValue getValue() {
			final Object arrayElement = accessArrayElement(this.array, this.index);
		...
		public void setValue(@Nullable final Object newValue) {
			final TypeDescriptor elementType = this.typeDescriptor.getElementTypeDescriptor();
	...
	private class MapIndexingValueRef implements ValueRef {
		...
		public MapIndexingValueRef(
				final TypeConverter typeConverter, final Map map, @Nullable final Object key, final TypeDescriptor mapEntryDescriptor) {
	...
	private class PropertyIndexingValueRef implements ValueRef {
		...
		public PropertyIndexingValueRef(final Object targetObject, final String value,
				final EvaluationContext evaluationContext, final TypeDescriptor targetObjectTypeDescriptor) {
		...
		@Override
		public TypedValue getValue() {
			final Class<?> targetObjectRuntimeClass = getObjectClass(this.targetObject);
			try {
				if ((Indexer.this.cachedReadName != null) && Indexer.this.cachedReadName.equals(this.name) &&
					...
					final PropertyAccessor accessor = Indexer.this.cachedReadAccessor;
					Assert.state(accessor != null, "No cached read accessor");
					return accessor.read(this.evaluationContext, this.targetObject, this.name);
				}
				final List<PropertyAccessor> accessorsToTry = AstUtils.getPropertyAccessorsToTry(
				...
				for (PropertyAccessor accessor : accessorsToTry) {
					if (accessor.canRead(this.evaluationContext, this.targetObject, this.name)) {
						...
						if (accessor instanceof ReflectivePropertyAccessor.OptimalPropertyAccessor) {
							final ReflectivePropertyAccessor.OptimalPropertyAccessor optimalAccessor = (ReflectivePropertyAccessor.OptimalPropertyAccessor) accessor;
							final Member member = optimalAccessor.member;
		...
		@Override
		public void setValue(@Nullable final Object newValue) {
			final Class<?> contextObjectClass = getObjectClass(this.targetObject);
			try {
				if ((Indexer.this.cachedWriteName != null) && Indexer.this.cachedWriteName.equals(this.name) &&
					...
					// It is OK to use the cached accessor
					final PropertyAccessor accessor = Indexer.this.cachedWriteAccessor;
	...
	private class CollectionIndexingValueRef implements ValueRef {
		...
		public CollectionIndexingValueRef(final Collection collection, final int index, final TypeDescriptor collectionEntryDescriptor,
				final TypeConverter typeConverter, final boolean growCollection, final int maximumSize) {
