...
function list_args()
{
  ...
  create_var_def('servers.0.networks.0.dhcp_mode', 'Boolean');
  create_var_def('servers.0.networks.0.fixed_ip', 'String');
...
// Formatting network array
if (isset($networks['dhcp_mode'])) {
   if ($networks['dhcp_mode'] !== true) {
      if (!empty($networks['fixed_ip'])) {
         $ret = prepare_json_response(FAILED, "As DHCP mode is checked - fixed IP address is mandatory.", $context, true);
         exit;
      }
      unset($network['dhcp_mode']);
   } else {
	unset($network['fixed_ip']); 
   }
}
